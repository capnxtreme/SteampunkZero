...
export class PhysicsSystem {
    ...
    private checkCollision(entityA: Entity, entityB: Entity): boolean {
        const colliderA = entityA.components.get('collider') as ColliderComponent | undefined;
        const colliderB = entityB.components.get('collider') as ColliderComponent | undefined;
        ...
        if (colliderA.type === 'box' && colliderB.type === 'box') {
            if (!colliderA.width || !colliderA.height || !colliderB.width || !colliderB.height) {
                return false;
            }
    ...
    private resolveCollision(entityA: Entity, entityB: Entity): void {
        ...
        // Calculate impulse
        const restitution = Math.min(physicsA.restitution || 0.5, physicsB.restitution || 0.5);
