...
export class Renderer {
  ...
  renderEntities(entities: Entity[], interpolation: number): void {
    ...
    const sortedEntities = entities
      ...
      .sort((a, b) => {
        const layerA =
          (a.components.get('render') as RenderComponent).layer || 0;
        const layerB =
          (b.components.get('render') as RenderComponent).layer || 0;
  ...
  private renderEntity(entity: Entity, interpolation: number): void {
    const render = entity.components.get('render') as
      | RenderComponent
      | undefined;
  ...
  screenToWorld(screenX: number, screenY: number): { x: number; y: number } {
    ...
    const worldY =
      (screenY - this.canvas.height / 2) / this.camera.zoom + this.camera.y;
    return { x: worldX, y: worldY };
  }

  worldToScreen(worldX: number, worldY: number): { x: number; y: number } {
    const screenX =
      (worldX - this.camera.x) * this.camera.zoom + this.canvas.width / 2;
    const screenY =
      (worldY - this.camera.y) * this.camera.zoom + this.canvas.height / 2;
