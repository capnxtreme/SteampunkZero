...
export class ErrorMonitor {
  ...
  private setupErrorHandlers(): void {
    ...
    window.addEventListener('error', (event) => {
      this.logError(`${event.message} at ${event.filename}:${event.lineno}:${event.colno}`, event.error?.stack);
  ...
  private logError(message: string, stack?: string): void {
    ...
    if (this.errorElement) {
      ...
      this.errorElement.innerHTML = this.errors
        ...
        .map(e => `<div style="margin-bottom: 5px; border-bottom: 1px solid rgba(255,255,255,0.2);">
  ...
  public getErrors(): Array<{ message: string; stack?: string; timestamp: number }> {
    return [...this.errors];
  }
